module.exports.start = function (app) {
  ...
  Seckill.find({fields: {id: true, seckillArrangements: true}}, function (err, data) {
    for (var i = 0; i < data.length; i++) {
      (function () {
        var seckillId = data[i].id;
        var seckillArrangments = data[i].seckillArrangements;
        ...
        SeckillResult.count({and: [{seckillId: seckillId}, {isGet: true}]}, function (err, resultCount) {
          ...
          var tempCount = 0;
          seckillArrangments.forEach(function (arrangement, index) {
                  ...
                  function (err, result) {
                    if (result)
                      ...
                      Seckill.findOne({
                        ...
                        if (new Date().getTime() <=
                          new Date(seckillArrangments[currentArrangementId].startTime).getTime())
                          //error: 还没有开始
                          socket.emit('killFail', 'not started');
                        else if (seckillRemain <= 0) {
                          //error: 没有余票了
                          socket.emit('killFail', 'no enough');
                          if (currentArrangementId < seckillArrangments.length) {
                            currentArrangementId++;
                            seckillRemain = seckillArrangments[currentArrangementId].total;
                          }
